...
def get_level_of_danger(x):
    return x ** 3 - 3 * x ** 2 - 12 * x + 10


# расчёт глубины
def calculate_safe_depth(level_dangerous):
    min_depth = 0
    max_depth = 4
    result = (min_depth + max_depth) / 2
    while abs(get_level_of_danger(result)) >= level_dangerous:
        if abs(get_level_of_danger(min_depth)) < abs(get_level_of_danger(max_depth)):
            max_depth = result
        else:
            min_depth = result
...
def main():
    accept_danger_level = float(
        input('Введите максимально допустимый уровень опасности: '))
    result = calculate_safe_depth(accept_danger_level)
